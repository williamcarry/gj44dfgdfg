  ...
  <FloorLayout title="平台爆款">
    <div class="mx-auto max-w-[1500px] min-w-[1200px] w-[80%]">
      ...
      <div v-for="p in platforms" :key="p.key" v-show="active === p.key">
          ...
          <ul class="flex flex-wrap" style="row-gap:10px;">
            ...
            >
              <div class="w-full flex flex-col items-center py-5">
                ...
                <!-- 产品标题 -->
                <a :href="it.href" target="_blank" class="px-2 text-sm text-slate-800 hover:text-primary line-clamp-2">
                  {{ it.title }}
                </a>
              </div>
            </li>
        ...
        <div v-else class="bg-white border border-slate-300 rounded py-20 text-center text-slate-500">
          {{ p.label }} 平台商品即将上线，敬请期待…
        </div>
      </div>
    </div>
  </FloorLayout>
